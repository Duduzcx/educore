...
import { Loader2, BookCheck, Target, Award, RotateCw, BrainCircuit, Library } from 'lucide-react';
...
type Question = {
  ...
  subjects: { name: string } | null; // Updated to match new schema
  year: number;
};
...
export default function SimuladoPage() {
  ...
  if (gameState === 'error') {
    return <div className='text-center text-red-500'>Ocorreu um erro. Por favor, atualize a página.</div>
  }
                ...
                <Card key={subject.id} className={`border-none shadow-xl rounded-[2.5rem] bg-white overflow-hidden flex flex-col justify-between ${subject.question_count === 0 ? 'opacity-50' : ''}`}>
                    ...
                    <CardContent>
                        <Button 
                            ...
                            disabled={subject.question_count < SIMULATION_SIZE}
                            className='w-full h-12 rounded-xl bg-primary font-black'
                        >
                            {subject.question_count < SIMULATION_SIZE ? `Requer ${SIMULATION_SIZE} questões` : 'Começar Simulado'}
                        </Button>
                    </CardContent>
                </Card>
            ))}
