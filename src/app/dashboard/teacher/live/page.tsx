...
export default function TeacherLiveManagement() {
  ...
  const fetchData = async () => {
    ...
    try {
      const { data: livesData, error: lError } = await supabase
        .from('lives')
        .select('*')
  ...
  const handleCreateLive = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setLives([data, ...lives]);
      toast({ title: "Transmissão Agendada!", description: "O sinal será liberado no horário marcado." });
      setForm({ title: "", description: "", youtube_id: "", start_time: "", trail_id: "none" });
      setIsAddOpen(false);
    } catch (err: any) {
      toast({ variant: "destructive", title: "Erro no Estúdio", description: "Verifique a estrutura da tabela 'lives'." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSeedLives = async () => {
    ...
    try {
      const demoLives = [
        ...
        {
          title: "Workshop de Física Quântica",
          description: "Entendendo o universo subatômico.",
          teacher_name: user.user_metadata?.full_name || "Mentor",
          teacher_id: user.id,
          youtube_id: "rfscVS0vtbw",
          youtube_url: "https://www.youtube.com/watch?v=rfscVS0vtbw",
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!error) {
      ...
      toast({ title: "Sinal Encerrado", description: "A live foi removida do sistema." });
    }
  };
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in duration-700 pb-20 max-w-[1600px] mx-auto">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl md:text-5xl font-black text-white italic tracking-tighter">STUDIO CORE</h1>
            </div>
            <p className="text-white/60 font-medium max-w-xl italic text-sm md:text-lg">
              Gerencie suas transmissões em tempo real. Conecte-se com milhares de alunos através de lives integradas às suas trilhas pedagógicas.
            </p>
          </div>
              ...
              <DialogContent className="rounded-[2.5rem] p-10 max-w-lg bg-white border-none shadow-2xl">
                ...
                <form onSubmit={handleCreateLive} className="space-y-6 py-6">
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase opacity-40">Vincular à Trilha (Destino)</Label>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="youtube.com/watch?v=..." value={liveForm.youtube_id} onChange={(e) => setForm({...liveForm, youtube_id: e.target.value})} className="h-14 pl-12 rounded-2xl bg-muted/30 border-none font-medium" required />
                    </div>
                  </div>
                  <Button type="submit" disabled={loading} className="w-full h-16 bg-red-600 text-white font-black text-lg rounded-2xl shadow-xl active:scale-95 transition-all">
      ...
      {/* PAINEL DE CONTROLE DE LIVES */}
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
        {/* LADO ESQUERDO: LISTA DE MONITORAMENTO */}
        <div className="xl:col-span-3 space-y-6">
          ...
          {livesLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card key={live.id} className="group border-none bg-white rounded-[2rem] overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 flex flex-col">
                  <div className="relative aspect-video bg-black overflow-hidden">
                    ...
                    <div className="absolute top-4 left-4 flex gap-2">
                      ...
                      {live.trail_id && (
                        <Badge className="bg-white/20 backdrop-blur-md text-white border-none font-black text-[8px] uppercase px-3 py-1">
                          VINCULADA
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-black/40 backdrop-blur-sm">
                       <Button variant="outline" className="rounded-full h-16 w-16 border-white/20 bg-white/10 hover:bg-red-600 hover:border-red-600 text-white transition-all scale-90 group-hover:scale-100" asChild>
                         <a href={`https://youtube.com/watch?v=${live.youtube_id}`} target="_blank" rel="noopener noreferrer">
                  ...
                  <CardContent className="p-6 flex items-center justify-between bg-white mt-auto">
                    <div className="flex flex-col">
                      ...
                      <p className="text-xs font-bold text-primary truncate max-w-[150px]">
                        {trails.find(t => t.id === live.trail_id)?.title || 'Mural de Lives'}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl hover:bg-primary/5 text-primary/40 hover:text-primary transition-all group/btn" asChild>
                        <a href={`https://youtube.com/watch?v=${live.youtube_id}`} target="_blank" rel="noopener noreferrer">
                          <ExternalLink className="h-5 w-5 group-hover/btn:scale-110" />
                        </a>
                      </Button>
                      <Button variant="ghost" size="icon" onClick={() => handleDelete(live.id)} className="h-10 w-10 rounded-xl hover:bg-red-50 text-muted-foreground hover:text-red-600 transition-all group/del">
                        <Trash2 className="h-5 w-5 group-hover/del:rotate-12" />
                      </Button>
                    </div>
                  </CardContent>
                </Card>
              ...
              {lives.length === 0 && (
                <div className="col-span-full py-32 text-center border-4 border-dashed border-muted/20 rounded-[3rem] bg-muted/5 opacity-40">
                  <Video className="h-16 w-16 mx-auto mb-4 text-muted-foreground" />
                  <p className="font-black italic text-xl text-primary">Estúdio em Silêncio</p>
                  <p className="text-sm font-medium mt-2">Clique em "Nova Transmissão" para iniciar seu show.</p>
                </div>
              )}
            </div>
          )}
          ...
          <Card className="border-none bg-white rounded-[2.5rem] p-8 shadow-xl relative overflow-hidden group">
            <div className="relative z-10 space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase text-muted-foreground">Potência de Rede</p>
                  <p className="text-xl font-black italic">Sinal Forte</p>
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex justify-between items-center text-[10px] font-bold uppercase opacity-60">
                  <span>Qualidade de Áudio</span>
                  <span className="text-green-600">HD 320kbps</span>
                </div>
          ...
          <Card className="border-none bg-primary text-white rounded-[2.5rem] p-8 shadow-2xl overflow-hidden relative">
            ...
            <div className="relative z-10 space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] font-black uppercase text-white/40">Interatividade</p>
                  <p className="text-xl font-black italic">Chat Integrado</p>
                </div>
              </div>
              <p className="text-[11px] font-medium leading-relaxed italic text-white/60">
                O chat das suas lives agora é conectado ao sistema de fóruns da rede EduCore. 
                Isso permite que as dúvidas dos alunos sejam salvas automaticamente para consulta posterior.
              </p>
              <Button variant="outline" className="w-full rounded-xl border-white/20 text-white hover:bg-white/10 font-black h-12 uppercase text-[10px] tracking-widest">
                Abrir Moderador
              </Button>
            </div>
          </Card>

          <div className="p-8 bg-muted/5 rounded-[2.5rem] border-2 border-dashed border-primary/10 space-y-4">
            <div className="flex items-center gap-3">
              <Settings2 className="h-5 w-5 text-primary/40" />
              <span className="text-[10px] font-black uppercase text-primary/40 tracking-widest">Quick Tips</span>
            </div>
            <p className="text-[11px] font-medium italic text-muted-foreground leading-relaxed">
              Para uma melhor experiência, utilize o ID do YouTube em vez da URL completa ao agendar novas transmissões.
            </p>
          </div>
        </div>
      </div>
